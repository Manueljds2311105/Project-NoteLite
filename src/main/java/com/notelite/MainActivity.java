    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        adapter.setOnItemClickListener(note -> {
            if (currentMode == 3) {
                ...
                // --- UPDATE: Kirim data Waktu ke AddNoteActivity ---
    ...
    private void loadNotesFromDatabase() {
        ...
        if (cursor.getCount() == 0) {
            ...
            while (cursor.moveToNext()) {
                ...
                // --- UPDATE: Baca kolom last_edited (Index 8) ---
                long lastEdited = 0;
                if(cursor.getColumnCount() > 8) lastEdited = cursor.getLong(8);

                noteList.add(new Note(id, title, content, isBookmarked, isArchived, isDeleted, tags, lastEdited));
            }
