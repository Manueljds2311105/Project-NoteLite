...
public class SearchActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        adapter.setOnItemClickListener(note -> {
            ...
            // --- UPDATE PENTING: Kirim data Tags saat dibuka dari pencarian ---
            intent.putExtra("extra_tags", note.getTags());

            // --- UPDATE BARU: Kirim data Waktu ---
            intent.putExtra("extra_last_edited", note.getLastEdited());
    ...
    private void searchNotes(String keyword) {
        ...
        if (cursor != null && cursor.getCount() > 0) {
            ...
            while (cursor.moveToNext()) {
                ...
                // Ambil data Tags
                String tags = "";
                if(cursor.getColumnCount() > 7) tags = cursor.getString(7);

                // --- UPDATE BARU: Ambil data Waktu dari index 8 ---
                long lastEdited = 0;
                if(cursor.getColumnCount() > 8) lastEdited = cursor.getLong(8);

                // Masukkan 8 parameter
                noteList.add(new Note(id, title, content, isBookmarked, isArchived, false, tags, lastEdited));
            }
        } else {
